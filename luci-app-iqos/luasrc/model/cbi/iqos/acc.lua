...
m = Map("iqos", translate("Active Congestion Control"),
...
m.redirect = dsp.build_url("admin/qos/iqos")
m.submit = translate("Save & Apply")
m.reset = false

-- 获取带宽设置
function get_bandwidth_settings()
    local dl_config = uci:get_all("iqos", "download") or {}
    local ul_config = uci:get_all("iqos", "upload") or {}
...
function generate_qosacc_args()
    local config = uci:get_all("iqos", "qosacc") or {}
    local bandwidth = get_bandwidth_settings()
    local args = {}

    -- 获取WAN接口：从global节的wan_interface读取（必须存在）
    local wan_iface = uci:get("iqos", "global", "wan_interface")
...
function status_display.cfgvalue()
    ...
    -- 然后获取配置中的enabled状态
    local config_enabled = uci:get("iqos", "qosacc", "enabled") or "0"
...
function m.on_init(self)
    -- 确保qosacc节存在
    if not uci:get("iqos", "qosacc") then
        uci:section("iqos", "qosacc", "qosacc", {
            enabled = "0",
            use_ipv6 = "0",
            ping_target = "",
            ping_target_v6 = "",
