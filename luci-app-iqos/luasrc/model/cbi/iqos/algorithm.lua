...
local m = Map("iqos", translate("QoS Algorithm Configuration"),
...
local function get_active_algorithm()
	local global_algo = uci:get("iqos", "global", "algorithm")
...
function m.on_after_apply(self)
	uci:save("iqos")
	uci:commit("iqos")
	os.execute("/etc/init.d/iqos restart 2>/dev/null")
	http.redirect(dsp.build_url("admin/qos/iqos/algorithm"))
end
