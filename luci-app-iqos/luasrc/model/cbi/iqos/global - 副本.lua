...
local function qos_enabled()
    local enabled = uci:get(iqos, "global", "enabled")
    return enabled == "1"
end
...
local function after_apply(self)
    sys.call("logger -t iqos '配置已应用，正在处理服务启停'")
    
    -- 1. 同步增强功能开关到各自的配置节（确保服务启动时能读取到最新值）
    local ack_enabled = self.uci:get(iqos, "global", "enable_ack_limit") or "1"
    local tcp_enabled = self.uci:get(iqos, "global", "enable_tcp_upgrade") or "1"
    local udp_enabled = self.uci:get(iqos, "global", "enable_udp_limit") or "1"

    if not self.uci:get(iqos, "ack_limit") then
        self.uci:set(iqos, "ack_limit", "ack_limit")
    end
    self.uci:set(iqos, "ack_limit", "enabled", ack_enabled)

    if not self.uci:get(iqos, "tcp_upgrade") then
        self.uci:set(iqos, "tcp_upgrade", "tcp_upgrade")
    end
    self.uci:set(iqos, "tcp_upgrade", "enabled", tcp_enabled)

    if not self.uci:get(iqos, "udp_limit") then
        self.uci:set(iqos, "udp_limit", "udp_limit")
    end
    self.uci:set(iqos, "udp_limit", "enabled", udp_enabled)

    local dynamic_enabled = self.uci:get(iqos, "global", "enable_dclassify") or "1"
    self.uci:set(iqos, "global", "enable_dclassify", dynamic_enabled)

    self.uci:commit(iqos)   -- 提交所有更改

    -- 2. 根据启用状态控制服务
    local enabled = self.uci:get(iqos, "global", "enabled") or "0"
    if enabled == "1" then
...
function m.parse(self, ...)
    ...
    local save = luci.http.formvalue("cbi.cbid.iqos.global.enabled")
    
    if apply then
        sys.call("logger -t iqos '检测到应用按钮点击'")
        after_apply(self)
    end
...
function m.write(self, section, value)
    sys.call("logger -t iqos_debug 'write 函数被调用，section: ' .. tostring(section) .. ', value: ' .. tostring(value)")
